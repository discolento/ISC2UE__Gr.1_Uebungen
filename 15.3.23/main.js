...
function show(value){
    ...
    Geboren Monat: ${value.birth.getMonth()}<br>
    Geboren Tag: ${value.birth.getDay()}<br>
    Button: ${value.button}<br><br>
    Zeichenlänge Name: ${value.name.length}<br>
    Zeichen auf einer Random Position:
    ${value.name.charAt(Math.floor(Math.random()*value.name.length))}<br>`;
}
